...
local function map(mode, key, cmd, opts, defaults)
  ...
  if type(cmd) == "function" then
    table.insert(M.__functions, cmd)
    if opts.expr then
      cmd = ([[luaeval('require("utils.map").execute(%d)')]]):format(#M.__functions)
    else
      cmd = ("<Cmd>lua require('utils.map').execute(%d)<CR>"):format(#M.__functions)
    end
  end
...
function M.default_map(mode, key, cmd, opt)
  return map(mode, key, cmd, opt)
end

function M.map(key, cmd, opt)
  return map("", key, cmd, opt)
end
...
function M.default_noremap(mode, key, cmd, opts)
