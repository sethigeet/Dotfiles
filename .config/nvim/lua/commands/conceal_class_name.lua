...
local conceal_ns = vim.api.nvim_create_namespace("conceal_class_names")

local function conceal_class_names(event)
  ...
  local ok, query = pcall(
    ...
    [[
(jsx_attribute
    (property_identifier) @attr_name (#eq? @attr_name "className")
    (string (string_fragment) @attr_value)
)
    ]]
  )
  if not ok then log.error(query) end
...
vim.api.nvim_create_user_command("ConcealClassNames", conceal_class_names, {
  desc = "Running this command will hide the class names which is ideal for TailwindCSS",
})
