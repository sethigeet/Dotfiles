    ...
    for i, p in pairs({...}) do
        if p ~= "" then
            if is_absolute_path(p) then
                ...
                result = (result ~= "") and (result:gsub("[\\" .. sep .. "]*$", "") .. sep .. p) or
                             p
            end
        end
    end
...
function clear_table(target) for key, value in pairs(target) do target[key] = nil end end
...
function ExecutableFinder:get_executable_path(name, raw_name)
    ...
    if self.path_cache[name] == nil then self.path_cache[name] = find_executable(name) or false end
...
function format_time(seconds, sep, decimals)
    ...
    local second_format = string.format("%%0%d.%df", 2 + (decimals > 0 and decimals + 1 or 0),
                                        decimals)
...
function format_time_hms(seconds, sep, decimals, force_full)
    ...
    if force_full or h > 0 then
        return string.format("%dh" .. sep .. "%dm" .. sep .. "%." .. tostring(decimals) .. "fs", h,
                             m, s)
    elseif m > 0 then
...
function extend_table(target, source) for i, v in pairs(source) do table.insert(target, v) end end
...
local sha1 = (function()
    ...
    local function w32_to_bytes(i)
        return floor(i / 0x1000000) % 0x100, floor(i / 0x10000) % 0x100, floor(i / 0x100) % 0x100,
               i % 0x100
    end
    ...
    local function byte_to_bit_string(b) return bits_to_string(byte_to_bits(b)) end
    ...
    local function w32_to_bit_string(a)
        ...
        return ("%s %s %s %s"):format(s(aa):reverse(), s(ab):reverse(), s(ac):reverse(),
                                      s(ad):reverse()):reverse()
    end
    ...
    local function w32_or3(a, b, c)
        ...
        return bytes_to_w32(bor(aa, bor(ba, ca)), bor(ab, bor(bb, cb)), bor(ac, bor(bc, cc)),
                            bor(ad, bor(bd, cd)))
    end
    ...
    function sha1.hex(msg)
        ...
        while chunk < chunks do
            ...
            H0, H1, H2, H3, H4 = w32_add(H0, A), w32_add(H1, B), w32_add(H2, C), w32_add(H3, D),
                                 w32_add(H4, E)
        end
    ...
    function sha1.hmacHex(key, text)
        ...
        local key_xord_with_0x36 = key:gsub('.', xor_with_0x36) ..
                                       string.rep(string.char(0x36), blocksize - #key)
        local key_xord_with_0x5c = key:gsub('.', xor_with_0x5c) ..
                                       string.rep(string.char(0x5c), blocksize - #key)

        return sha1.hex(key_xord_with_0x5c .. sha1.bin(key_xord_with_0x36 .. text))
    end

    function sha1.hmacBin(key, text) return hex_to_binary(sha1.hmacHex(key, text)) end
...
function Thumbnailer:on_video_change(params)
    ...
    if params ~= nil then if not self.state.ready then self:update_state() end end
end
...
function Thumbnailer:update_state()
    ...
    if has_video and self.state.thumbnail_delta ~= nil and self.state.thumbnail_size ~= nil and
        self.state.thumbnail_count > 0 then self.state.available = true end
...
function Thumbnailer:get_delta()
    ...
    if is_remote and thumbnailer_options.thumbnail_network then remote_and_disallowed = false end
...
function Thumbnailer:get_thumbnail_index(time_position)
    ...
    if self.state.thumbnail_delta and
        ...
        return math.min(math.floor(time_position / self.state.thumbnail_delta) + 1,
...
function Thumbnailer:register_client()
    ...
    mp.register_script_message("mpv_thumbnail_script-ready",
                               function(index, path) self:on_thumb_ready(tonumber(index), path) end)
    ...
    mp.observe_property("video-dec-params", "native", function()
        ...
        if self.state.available and thumbnailer_options.autogenerate then
            ...
            if duration < max_duration or max_duration == 0 then self:start_worker_jobs() end
        end
    end)
...
function Thumbnailer:start_worker_jobs()
    ...
    if worker_count == 0 then
        ...
        if mp.get_time() > self.worker_register_timeout then
            ...
            self.worker_wait_timer = mp.add_timeout(wait_time,
                                                    function() self:start_worker_jobs() end)
        end
        ...
        for worker_name, worker_frames in pairs(worker_jobs) do
            if #worker_frames > 0 then
                ...
                mp.commandv("script-message-to", worker_name, "mpv_thumbnail_script-job",
                            state_json_string, frames_json_string)
            end
        end
    end
end
...
mp.observe_property("video-dec-params", "native",
                    function(name, params) Thumbnailer:on_video_change(params) end)
--[[
...
function display_thumbnail(pos, value, ass)
    ...
    if not ((duration == nil) or (value == nil)) then
        ...
        if display_progress then
            ...
            for i, v in pairs(Thumbnailer.state.thumbnails) do
                if i ~= closest_index and v > 0 then
                    ass:rect_cw((i - 1) * block_w, 0, math.min(block_max_x, i * block_w),
                                framegraph_h)
                end
            end
            ass:draw_stop()
            ...
            if closest_index ~= nil then
                ...
                ass:rect_cw((closest_index - 1) * block_w, 0,
                            math.min(block_max_x, closest_index * block_w), framegraph_h)
                ass:draw_stop()
            end
        end
        ...
        if thumb_path then
            ...
            if not (osc_thumb_state.last_path == thumb_path and osc_thumb_state.last_x == thumb_x and
                osc_thumb_state.last_y == thumb_y) then
                local overlay_add_args = {
                    "overlay-add", osc_thumb_state.overlay_id, thumb_x, thumb_y, thumb_path, 0,
                    "bgra", thumb_size.w, thumb_size.h, 4 * thumb_size.w
                }
...
function get_slider_ele_pos_for(element, val)
    ...
    local ele_pos = scale_value(element.slider.min.value, element.slider.max.value,
...
function get_slider_value_at(element, glob_pos)
    ...
    local val = scale_value(element.slider.min.glob_pos, element.slider.max.glob_pos,
...
function get_slider_value(element) return get_slider_value_at(element, get_virt_mouse_pos()) end
...
function get_tracklist(type)
    ...
    if #tracks_osc[type] == 0 then
        ...
        for n = 1, #tracks_osc[type] do
            ...
            if (track.id == tonumber(mp.get_property(type))) then selected = "●" end
...
function prepare_elements()
    ...
    for _, element in pairs(elements) do
        ...
        elseif (element.type == "slider") then
            ...
            -- the "hole"
            static_ass:rect_ccw(slider_lo.border, slider_lo.border, elem_geo.w - slider_lo.border,
...
function render_elements(master_ass)
    ...
    for n = 1, #elements do
        ...
        style_ass:append(string.format("{\\1a&H%X&\\2a&H%X&\\3a&H%X&\\4a&H%X&}", ar[1], ar[2],
                                       ar[3], ar[4]))
        ...
        if element.eventresponder and (state.active_element == n) then
            ...
            if mouse_hit(element) then
                ...
                if (element.softrepeat) and
                    (state.mouse_down_counter >= 15 and state.mouse_down_counter % 5 == 0) then
        ...
        if (element.type == "slider") then
            ...
            if not (pos == nil) then
                ...
                elseif (slider_lo.stype == "knob") then
                    elem_ass:rect_cw(xp, (9 * innerH / 20) + foV, elem_geo.w - foH,
                                     (11 * innerH / 20) + foV)
                    elem_ass:rect_cw(foH, (3 * innerH / 8) + foV, xp, (5 * innerH / 8) + foV)
                    elem_ass:round_rect_cw(xp - innerH / 2, foV, xp + innerH / 2, foV + innerH,
                                           innerH / 2.0)
                end
            end
            ...
            if not (element.slider.tooltipF == nil) then
                ...
                if mouse_hit(element) then
                    ...
                    elem_ass:append(string.format("{\\1a&H%X&\\2a&H%X&\\3a&H%X&\\4a&H%X&}", ar[1],
                                                  ar[2], ar[3], ar[4]))

                    elem_ass:append(tooltiplabel)

                    -- mpv_thumbnail_script.lua --
                    display_thumbnail({x = get_virt_mouse_pos(), y = ty, a = an}, sliderpos,
                                      elem_ass)
                    -- // mpv_thumbnail_script.lua // --
        ...
        elseif (element.type == "button") then
            ...
            if not (maxchars == nil) and (#buttontext > maxchars) then
                ...
                buttontext = string.format("{\\fscx%f}", (maxchars / #buttontext) * 100) ..
                                 buttontext
            end
...
layouts["slimbox"] = function()
    ...
    lo.alpha[1] = user_opts.boxalpha
    lo.alpha[3] = 0
    if not (user_opts["seekbarstyle"] == "bar") then lo.box.radius = osc_geo.r end
...
layouts["bottombar"] = function()
    ...
    lo = add_layout("title")
    lo.geometry = geo
    lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.vidtitleBar, geo.x,
                             geo.y - geo.h, geo.w, geo.y + geo.h)
...
function osc_init()
    ...
    ne.content = function()
        ...
        if (is_network and dmx_cache) or show_cache then
            ...
            return string.format("Cache: %s%s%s", (dmx_cache and dmx_cache or ""),
...
function render()
    ...
    if not (state.mp_screen_sizeX == current_screen_sizeX and state.mp_screen_sizeY ==
        current_screen_sizeY) then
    ...
    if not (state.anitype == nil) then
        ...
        if (now < state.anistart + (user_opts.fadeduration / 1000)) then
            ...
            elseif (state.anitype == "out") then -- fade out
                state.animation = scale_value(state.anistart,
                                              (state.anistart + (user_opts.fadeduration / 1000)), 0,
                                              255, now)
            end
    ...
    if not (state.showtime == nil) and (user_opts.hidetimeout >= 0) and
        (state.showtime + (user_opts.hidetimeout / 1000) < now) and (state.active_element == nil) and
        not (mouse_over_osc) then hide_osc() end
    ...
    -- mpv_thumbnail_script.lua --
    if not osc_thumb_state.visible and thumb_was_visible then hide_thumbnail() end
    -- // mpv_thumbnail_script.lua // --
...
function enable_osc(enable)
    ...
    if enable then
        ...
        if state.showhide_enabled then mp.disable_key_bindings("showhide") end
        state.showhide_enabled = false
    end
end
...
mp.set_key_bindings({
    {"mouse_move", function(e) process_event("mouse_move", nil) end}, {"mouse_leave", mouse_leave}
}, "showhide", "force")
do_enable_keybindings()
...
mp.set_key_bindings({
    ...
    {"mbtn_left_dbl", "ignore"}, {"shift+mbtn_left_dbl", "ignore"}, {"mbtn_right_dbl", "ignore"}
}, "input", "force")
mp.enable_key_bindings("input")
