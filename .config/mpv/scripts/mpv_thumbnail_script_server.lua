...
function create_thumbnail_mpv(file_path, timestamp, size, output_path, options)
    ...
    local mpv_command = skip_nil({
        ...
        (ytdl_disabled and "--no-ytdl" or nil), -- Pass HTTP headers from current instance
        header_fields_arg, -- Pass User-Agent and Referer - should do no harm even with ytdl active
        "--user-agent=" .. mp.get_property_native("user-agent"),
        "--referrer=" .. mp.get_property_native("referrer"), -- Disable hardware decoding
        "--hwdec=no", -- Insert --no-config, --profile=... and --log-file if enabled
        (thumbnailer_options.mpv_no_config and "--no-config" or nil), profile_arg,
        (thumbnailer_options.mpv_logs and log_arg or nil), file_path,

        "--start=" .. tostring(timestamp), "--frames=1", "--hr-seek=yes", "--no-audio",
        -- Optionally disable subtitles
        (thumbnailer_options.mpv_no_sub and "--no-sub" or nil),

        ("--vf=scale=%d:%d"):format(size.w, size.h), "--vf-add=format=bgra", "--of=rawvideo",
        "--ovc=rawvideo", "--o=" .. output_path
    })
...
function create_thumbnail_ffmpeg(file_path, timestamp, size, output_path)
    local ffmpeg_command = {
        "ffmpeg", "-loglevel", "quiet", "-noaccurate_seek", "-ss", format_time(timestamp, ":"),
        "-i", file_path, "-frames:v", "1", "-an", "-vf", ("scale=%d:%d"):format(size.w, size.h),
        "-c:v", "rawvideo", "-pix_fmt", "bgra", "-f", "rawvideo", "-y", output_path
    }
...
function do_worker_job(state_json_string, frames_json_string)
    ...
    local generate_thumbnail_for_index = function(thumbnail_index)
        ...
        local timestamp = math.min(file_duration - 0.25,
        ...
        if need_thumbnail_generation then
            local ret = thumbnail_func(file_path, timestamp, thumb_state.thumbnail_size,
                                       thumbnail_path, thumb_state.worker_extra)
            local success =
                check_output(ret, thumbnail_path, thumbnail_func == create_thumbnail_mpv)
        ...
        mp.commandv("script-message", "mpv_thumbnail_script-ready", tostring(thumbnail_index),
                    thumbnail_path)
    end

    msg.debug(("Generating %d thumbnails @ %dx%d for %q"):format(#thumbnail_indexes,
